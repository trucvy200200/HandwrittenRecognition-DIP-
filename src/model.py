...
class Model:
    ...
    def __init__(self,
        ...
        with tf.compat.v1.control_dependencies(self.update_ops):
    ...
    def setup_cnn(self) -> None:
        """Create CNN layers."""
        cnn_in4d = tf.compat.v1.expand_dims(input=self.input_imgs, axis=3)
        ...
        for i in range(num_layers):
            kernel = tf.compat.v1.Variable(
                tf.compat.v1.random.truncated_normal([kernel_vals[i], kernel_vals[i], feature_vals[i], feature_vals[i + 1]],
                                           stddev=0.1))
            conv = tf.compat.v1.nn.conv2d(input=pool, filters=kernel, padding='SAME', strides=(1, 1, 1, 1))
    ...
    def setup_ctc(self) -> None:
        ...
        if self.decoder_type == DecoderType.BestPath:
            self.decoder = tf.compat.v1.nn.ctc_greedy_decoder(inputs=self.ctc_in_3d_tbc, sequence_length=self.seq_len)
        elif self.decoder_type == DecoderType.BeamSearch:
            self.decoder = tf.compat.v1.nn.ctc_beam_search_decoder(inputs=self.ctc_in_3d_tbc, sequence_length=self.seq_len,
                                                         beam_width=50)
        # word beam search decoding (see https://github.com/githubharald/CTCWordBeamSearch)
            ...
            self.wbs_input = tf.compat.v1.nn.softmax(self.ctc_in_3d_tbc, axis=2)
